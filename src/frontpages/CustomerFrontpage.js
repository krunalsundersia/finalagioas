...
const Customer = ({ activeSim, onBack }) => {
    ...
    const [instructionsChecked, setInstructionsChecked] = useState(() => {
        try { return JSON.parse(localStorage.getItem('asklurk_customer_instructions') || 'false'); } catch { return false; }
    });

    // Save instructionsChecked to localStorage when it changes
    useEffect(() => {
        localStorage.setItem('asklurk_customer_instructions', JSON.stringify(instructionsChecked));
    }, [instructionsChecked]);
    ...
    useEffect(() => {
        try {
            const savedConfig = localStorage.getItem('asklurk_customer_config');
            if (savedConfig) {
    ...
    // Auto-save ALL config fields to localStorage whenever config changes
    useEffect(() => {
        try {
            localStorage.setItem('asklurk_customer_config', JSON.stringify(config));
        } catch (e) {
    ...
    const handleTitleChange = (e) => {
        ...
        // config is now auto-saved via the useEffect above
    };
    ...
    const handleStart = () => {
        ...
        // ✅ BUILD COMPLETE COMPANY CONTEXT — uses actual config state field names
        const companyContext = {
            // Product Review Setup
            title: config.title,
            productName: config.title,                        // title doubles as product name
            productPitch: config.pitchText,                   // ← was config.productPitch (undefined)
            valueProp: config.valueProp,
            visualDescription: config.productDesc,            // ← was config.visualDescription (undefined)
            meetingMode: config.meetingMode,

            // ICP / Target
            targetJourney: config.targetJourney,
            targetArea: config.targetArea,
            targetPocket: config.targetPocket,

            // Product Review Prerequisites
            preReviewContext: config.preReviewContext,         // ← was missing
            knownLimitations: config.knownLimitations,        // ← was missing
            competitorContext: config.competitorContext,       // ← was missing (competitors)
            successMetrics: config.successMetrics,            // ← was missing
